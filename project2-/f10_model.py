...
class User(db.Model):
	...
	def __init__(self, username, email):   # (mej)redefine the initializer       (if you hate the default initializer which requuires you to type username =...., email = ...);  but this one will still automatically populate value of ID
...
def initdb_command():  #(mej) not using app.route(...) but giving command-line a way to set up database
	"""Reinitializes the database"""
	db.drop_all()   #(mej) get rid of all tables
	db.create_all() #(mej) create all the tables that have been defined so far
